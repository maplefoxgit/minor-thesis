...
| E5 Practical overhead | Report modest local timing, CPU, memory, file-size, rule-count, and graph-size evidence | `experiments/run_e5_overhead.py`, overhead metrics output | `results/metrics/overhead_metrics.json`, experiment summary |
...
- The overhead evidence reports modest local proof-of-concept cost only and does not claim production scalability.
